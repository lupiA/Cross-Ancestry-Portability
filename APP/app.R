...
server <- function(input, output, session) {
  ...
  read_csv_files <- function(dataset, end, ancestry) {
    
    tmp <- list()
    for (i in 1:end) {
      url <- paste0("https://raw.githubusercontent.com/lupiA/Cross-Ancestry-Portability/main/csv_files/MAP_",dataset,"_", 
                    ancestry, i, ".csv")
  ...
  dat <- reactive({
    if(input$dataset.input == "UK Biobank Arrays") {
      if(input$ancestry == "African"){
        dat <- read_csv_files(dataset = "UKB", end = 2, ancestry = "af")
      } else if(input$ancestry == "Caribbean"){
        dat <- read_csv_files(dataset = "UKB", end = 2, ancestry = "cr")
      } else if(input$ancestry == "East Asian"){
        dat <- read_csv_files(dataset = "UKB", end = 2, ancestry = "ea")
      } else if(input$ancestry == "South Asian"){
        dat <- read_csv_files(dataset = "UKB", end = 2, ancestry = "sa")
      }
  ...
  output$table2 <- renderTable({
    if (!is.null(input$rs_id) && !is.null(input$ancestry) && !is.null(filtered_snp_data())) {
      if (!is.null(filtered_snp_data())) {
        modified_table <- filtered_snp_data()[, c("Relative Accuracy", 
                                                  paste0("Rsq. EU\u2192", ancestry_label()), 
                                                  "Rsq. EU\u2192EU",
                                                  "Target Ancestry", "SNP", "Chromosome", "BP position",  
                                                  "MC Error RA", 
                                                  "S.E. RA", 
                                                  paste0("MC Error Rsq. EU\u2192", ancestry_label()), 
                                                  paste0("S.E. Rsq. EU\u2192", ancestry_label()), 
